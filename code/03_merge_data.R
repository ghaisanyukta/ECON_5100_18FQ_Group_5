...
educ_gender_hh_head_employ <- educ_gender_hh_head_employ %>%
              select(nh, clust, highest_educ_level, is_male, employment_status, agricultural_activities)
...
agri_hh_educ_gender_employ <- merge(x = agri_data, y = educ_gender_hh_head_employ, by = c('clust','nh'), all.x = TRUE) %>%
              select(nh, clust, agri1c, hh_highest_educ = highest_educ_level, hh_is_male = is_male, employment_status, agricultural_activities)
...
agri_educ_gender_employ <- merge(x = agri_hh_educ_gender_employ, y = highest_educ, by = c('clust','nh'), all.x = TRUE) %>%
            select(nh, clust, agri1c, hh_highest_educ, highest_educ_level, hh_is_male, employment_status,agricultural_activities)
